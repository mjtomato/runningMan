...
function tick(event) {		//舞台逐帧逻辑处理函数
	...
	if (man.ground[0] && !cg && !mfcg) {
		...
		// if (man.ground.length >= 2 && man.ground[1].kind === 'C') {
		// 	console.log(man.sprite.y, man.endy, man.picsize().h);
		// 	console.table(man.ground)
		// }
		// if (man.ground.length >= 2) {
		// 	if (man.endy > man.ground[1].y + man.ground.h) {
		// 		man.endy = man.ground[0].y - man.picsize().h * 1;
		// 	} else {
		// 		man.endy = man.ground[1].y + man.ground.h;
		// 	}
		// } else {
			man.endy = man.ground[0].y - man.picsize().h * 1;
		// }
